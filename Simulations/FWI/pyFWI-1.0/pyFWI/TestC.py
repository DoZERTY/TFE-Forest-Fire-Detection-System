...
class TestFWIFunctions(unittest.TestCase):
    ...
    def testExample(self):
        ...
        print 
        print "TEMP,  RH,  WIND,  RAIN,  FFMC,  DMC,  DC,  ISI,  BUI,  FWI"
        ...
        for x in range(0,49):
            try:
                ...
                lib.FFMCcalc.argtypes = [ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double]
                lib.FFMCcalc.restype = ctypes.c_double
                ffmc = lib.FFMCcalc(inputs[x][2],inputs[x][3],inputs[x][4],inputs[x][5],results[x][0])
                
                lib.DMCcalc.argtypes = [ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_int]
                lib.DMCcalc.restype = ctypes.c_double
                dmc = lib.DMCcalc(inputs[x][2],inputs[x][3],inputs[x][5],results[x][1],inputs[x][0])
                
                lib.DCcalc.argtypes = [ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_int]
                lib.DCcalc.restype = ctypes.c_double
                dc = lib.DCcalc(inputs[x][2],inputs[x][5],results[x][2],inputs[x][0])
                
                lib.ISIcalc.argtypes = [ctypes.c_double, ctypes.c_double]
                lib.ISIcalc.restype = ctypes.c_double
                isi = lib.ISIcalc(inputs[x][4], ffmc)

                lib.BUIcalc.argtypes = [ctypes.c_double, ctypes.c_double]
                lib.BUIcalc.restype = ctypes.c_double
                bui = lib.BUIcalc(dmc,dc)
                
                lib.FWIcalc.argtypes = [ctypes.c_double, ctypes.c_double]
                lib.FWIcalc.restype = ctypes.c_double
                fwi = lib.FWIcalc(isi, bui)

                results[x+1] = [ffmc,dmc,dc,isi,bui,fwi]

                print inputs[x][2],inputs[x][3],inputs[x][5],results[x][1],inputs[x][0],ffmc,dmc,dc,isi,bui,fwi 
                ...
                # write the error to a file for further analysis
                self.write_error(results[x+1][5],outputs[x+1][5])
                final = final & self.nearlyEqual(results[x+1][5],outputs[x+1][5])
    ...
    def write_error(self, result_approx , true_output):
        '''write the error to a file for further analysis'''
        error = abs(result_approx - true_output)
        f = open('error.txt', 'a')
        f.write("{}, {}\n".format(repr(error), repr(true_output)))
        f.close()

    def nearlyEqual(self, a, b):
        '''usefull for comparing doubles that may not be identical but are close enough'''
        error = abs(a-b)
        if error < 3*pow(10,0):    # due to the conversion from float python to double c
            return True
        return False
